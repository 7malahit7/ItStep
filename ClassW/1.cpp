...
{                                   
	int day/*:5*/;             
	int month/*:4*/;                     // раскоментить для установления размеров для числовый значений
	int year/*:7*/;
	char mon_neme[15];
};
struct Person
...
int main()
{
	...
	date myBirthday;
	const int n=10;                                                          // берем 10 , хотя можно и другое число
	int day;
	int mounth;
	int yers;
	...
	for (int i = 0; i < n; i++)                        
	{
		while (1)
		{
			...
			cin >> spisok[i].myBirthday.day;
			cout << "Введите месяц своего рождения " << "\n";
			cin >> spisok[i].myBirthday.month;
			cout << "Введите год своего рождения " << "\n";
			cin >> spisok[i].myBirthday.year;
