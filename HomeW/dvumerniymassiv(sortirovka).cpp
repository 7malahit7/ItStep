...
void main()
{
	...
	int i, j;
	//Подсказки
	enum { CHOICE = 4, INPUT_FILENAME, INPUT_DIMENSIONS, INPUT_ELEMENTS, FILE_ERROR };
	//сообщение
	...
	else if (Answer == '3')
	{
		...
		for (i = 0; i < M; i++)
		{
			for (j = 0; j < N; j++)
			{
				arr[i][j] = rand() % (-30 - 30) + -30;
				cout << arr[i][j] << "\t";
			}
			cout << endl;
		}

		//сортировка    всех столбиков -> всех строк
